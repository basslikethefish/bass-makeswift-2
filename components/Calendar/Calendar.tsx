...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
  ...
  return (
    <Popover open={date !== undefined} onOpenChange={isOpen => !isOpen && setDate(undefined)}>
      <PopoverAnchor>
      ...
      <PopoverContent className="w-80">
        <div className="grid gap-4">
          <div className="space-y-2">
            <h4 className="font-medium leading-none">Schedule for</h4>
            <p className="text-sm text-muted-foreground">
              {date?.toDateString()} at 12:30 PM with Dr. Bass
            </p>
          </div>
          <Button
            onClick={async () => {
